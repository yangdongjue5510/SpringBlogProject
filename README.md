...
### 로그인 링크
a. 로그인 / 블로그 등록, 내 블로그로 가기 링크가 로그인 여부에 따라 서로 배타적으로 표시.
b. 사용자가 로그인에 성공하고 나면, 로그인 여부에 따라 블로그 등록 / 내 블로그로 가기가 배타적으로 표시
c. 로그인 링크는 bloglogin.jsp로 이동.
d. 블로그 등록링크는 blogcreate.jsp로 이동
e. 내 블로그로 가기 링크는 blogmain.jsp로 이동

### 검색 기능
a. 검색 조건은 블로그 제목, 태그, 블로거(회원이름)
b. 검색 결과가 없는 경우 테이블 자체가 보이면 안됨.
c. 검색된 블로그 제목을 클릭하면 해당 블로그 메인 화면으로 이동.

### 블로그 삭제 기능
a. ADMIN 권한을 가진 사람만 블로그를 삭제할 수 있음.
b. USER 권한을 가진 사용자는 삭제 칼럼 자체가 제공 안됨.

## 2. 로그인 화면 bloglogin.jsp

1. 로그인 버튼을 누르면 결과 여부 상관없이 index.jsp로 이동.
2. 로그인 성공하면 블로그 소유에 따라 블로그 생성이나 내블로그 가기 링크가 표시 (로그인링크 b참고)
3. 로그인 실패할 경우, 다시 로그인 화면으로 갈 수 있게 로그인 링크만 보임(로그인 링크 b참고)

## 3. 블로그 생성 화면 blogcreate.jsp

1. 블로그 제목을 입력하고 블로그 생성을 누르면 해당 회원의 블로그가 생성.
2. tag, cnt_display, post, status는 기본값 설정할 것.
3. 블로그가 만들어지면 자동으로 '미분류'카테고리가 존재하도록 함.
4. 인덱스 페이지로 이동 링크를 누르면 블로그를 만들지 않고 메인화면으로 이동
